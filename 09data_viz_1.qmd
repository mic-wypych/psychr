...
-   Lines: Lets say we want to make a line plot that will display the 10th, 50th and 90th quantile of percent of adults in poverty in all states:

    ```{r}
    #get the quantiles
    library(dplyr)
    pov_quantiles <- midwest %>%
      group_by(state) %>%
      summarize(q_10 = quantile(midwest$percadultpoverty, .1),
                q_50 = quantile(midwest$percadultpoverty, .5),
                q_90 = quantile(midwest$percadultpoverty, .9))
    #plot them
    plot(pov_quantiles$q_10 ~ pov_quantiles$state, ylim = c(0, 20))
    lines(1:5, pov_quantiles$q_10, col = 2, lwd = 2, lty = 2)
    lines(1:5, pov_quantiles$q_50, col = 4, lwd = 2, lty = 1)
    lines(1:5, pov_quantiles$q_90, col = 2, lwd = 2, lty = 2)
    ```
